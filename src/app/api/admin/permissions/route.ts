import { NextRequest, NextResponse } from "next/server";
import { JWTPayload } from "@/lib/auth";
import { protectApi, ADMIN_ONLY } from "@/lib/auth-middleware";
import { getContainer } from "@/config/container-helper";
import { TYPES } from "@/types";
import { PermissionController } from "@/controllers/PermissionController";
import { quickOptions } from "@/lib/cors-helper";

// GET: Obtener todos los permisos disponibles
async function getAllPermissionsHandler(
  request: NextRequest,
  user?: JWTPayload
) {
  try {
    const container = getContainer();
    const permissionController = container.get<PermissionController>(
      TYPES.PermissionController
    );

    return await permissionController.getAllPermissions(request);
  } catch (error) {
    console.error("Error en admin/permissions route:", error);
    return NextResponse.json(
      { error: "Error interno del servidor" },
      { status: 500 }
    );
  }
}

// POST: Crear un nuevo permiso
async function createPermissionHandler(
  request: NextRequest,
  user?: JWTPayload
) {
  try {
    const container = getContainer();
    const permissionController = container.get<PermissionController>(
      TYPES.PermissionController
    );

    return await permissionController.createPermission(request);
  } catch (error) {
    console.error("Error en admin/permissions POST route:", error);
    return NextResponse.json(
      { error: "Error interno del servidor" },
      { status: 500 }
    );
  }
}

// OPTIONS handler usando el helper reutilizable
export const OPTIONS = quickOptions("ADMIN-PERMISSIONS");

// Solo administradores pueden gestionar permisos
export const GET = ADMIN_ONLY(getAllPermissionsHandler);
export const POST = ADMIN_ONLY(createPermissionHandler);
